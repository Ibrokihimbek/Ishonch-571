...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(create: (context) => CategoriesBloc(CategoriesRepo(apiService: ApiService()))..add(FetchAllCategories()),
      child: Scaffold(
        ...
        backgroundColor: const Color(0xFFFFFFFF),
          leading: Padding(
            padding: EdgeInsets.only(left: width(context)*0.055,top: height(context)*0.024),
            child: SvgPicture.asset(AppImages.iconMenu,),
          ),
          actions: [
            Padding(
              padding: EdgeInsets.only(right: width(context)*0.014,top: height(context)*0.014),
              child: Container(
